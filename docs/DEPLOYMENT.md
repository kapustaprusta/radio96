...
### CI-проверка и ручной релиз из GitHub Actions

Push в `main` автоматически запускает backend-, frontend- и production-проверки,
но не публикует образы и не изменяет production. Полный релиз запускается только
вручную через workflow `Deploy production`.

Если поле `release_sha` пустое и образов текущего SHA `main` ещё нет, workflow
собирает и публикует `radio96-api`, `radio96-web` и `radio96-migrate`. Если весь
комплект уже существует, он используется повторно. Частично опубликованный
комплект считается ошибкой и не перезаписывается.

Для ручного отката укажите полный SHA предыдущего релиза. В этом режиме workflow
не пересобирает старый код и требует наличия всех трёх неизменяемых образов.
Один SHA всегда выбирает согласованный комплект, поэтому смешивание версий
невозможно.

Release workflow использует OIDC и не хранит авторизованный ключ сервисного
аккаунта. Для него должны быть настроены repository variables:
...
1. проверяет, что job подготовки образов завершился успешно, а deploy job уже
   ожидает VM;
2. повторно проверяет статус workflow, его SHA и выбранный SHA релиза;
3. убеждается, что обе ревизии принадлежат `origin/main`;
4. получает пароль PostgreSQL и LiveKit credentials из Lockbox;
5. атомарно создаёт `.env.production` и обновляет CA bundle;
6. скачивает три образа с тегом SHA, применяет миграции и обновляет контейнеры;
7. подтверждает релиз через `/versionz` и готовность через `/readyz`.
...
Единый ручной pipeline и выбор SHA требуют актуальной root-owned копии агента.
После первого обновления с этой возможностью обязательно повторно выполните
`production-deployer-install` до запуска любого следующего релиза.
...
ветку `main` и подтвердите запуск. Поле `release_sha` можно оставить пустым: тогда
workflow подготовит и опубликует текущий SHA `main`, если этого ещё не было. Для
ручного отката укажите полный lowercase SHA предыдущего релиза: существующие
образы будут проверены без повторной сборки. После успешной подготовки комплекта
VM-agent применит миграции и обновит контейнеры, а workflow до 20 минут будет
ждать подтверждение через health endpoints. Одновременно выполняется только один
production-релиз.

Динамического списка тегов в форме GitHub Actions нет. SHA для отката можно
скопировать из имени предыдущего успешного запуска `Deploy production`. Не
используйте `latest`: неизменяемый SHA однозначно определяет содержимое релиза.
...
необратимыми. Ручной выбор предыдущего SHA откатывает версии контейнеров, но не
отменяет уже применённые миграции. Поэтому предыдущая версия приложения должна
оставаться совместимой с текущей схемой БД. При ошибке workflow останавливается,
а причину следует смотреть в логе systemd. Последний успешный SHA сохраняется в
state directory для ручного восстановления.
