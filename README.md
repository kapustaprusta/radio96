...
Webhook и reconciler остаются следующими этапами. В production собранный SPA
раздаёт Caddy, а Go-приложение обслуживает API. До реализации lifecycle нельзя
считать одноразовость ссылки после завершения медиасессии полностью обеспеченной.
...
Production-конфигурация отделена от локального Compose и описана в
[docs/DEPLOYMENT.md](docs/DEPLOYMENT.md). Она собирает отдельные образы API,
